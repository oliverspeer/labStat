...
col.types <- c(rep(c("text", "numeric", "skip", "skip", "date", "text", "text"), 1), rep("numeric", col.count-7))

global.data <- read_excel(full_path, skip = 2, col_names = head, col_types = col.types)
...
AU.sum.data <- tidy.AU.data |> 
  group_by(Year_Month) |> 
  summarize(AU_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            AU_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
dxi.sum.data <- tidy.dxi.data |> 
  group_by(Year_Month) |> 
  summarize(DxI_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            DxI_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
            )
...
sum.data <- combined.data |> 
  group_by(Year_Month) |> 
  summarize(
    TxpUmsatz_CoreLab = sum(Taxpkt., na.rm = TRUE),
    Aufträge_Hochd. = n_distinct(a_Tagesnummer, na.rm = TRUE), 
    Anz_Patienten_sum = n_distinct(b_Fallnummer, na.rm = TRUE)
            )
...
totals <- data.frame(Year_Month = "Total", 
                     TxpUmsatz_CoreLab = colSums(sum.data.22[,2]), 
                     Aufträge_Hochd. = colSums(sum.data.22[,3]), 
                     Anz_Patienten_sum = colSums(sum.data.22[,4]))
sum.data.22 <- rbind(sum.data.22, totals)
...
sum.data.22 |>  
  ...
  bg(bg = "grey", part = "header") |> 
  set_header_labels(
    TxpUmsatz_CoreLab = "TxpUmsatz -Hochdurchsatz",
    Aufträge_Hochd. = "Aufträge",
...
```{r quarterly_corelab, echo=FALSE, message=FALSE, warning=FALSE}
sum.cases.cl.22 <- combined.data |> 
  mutate(Quarter = paste0("Q", quarter(floor_date(Datum, "quarter")), "_", Year = year(Datum))) |>
  filter(Year == 2022) |> 
  summarize(
    TxpUmsatz_CoreLab = sum(Taxpkt., na.rm = TRUE),
...
sum.cases.cl.22.piv <- sum.cases.cl.22 |> 
  pivot_longer(!Quarter, names_to = "Umsätze", values_to = "col2") %>% 
  pivot_wider(names_from = "Quarter", values_from = "col2")

sum.cases.cl.22.piv |> 
  arrange(desc(Q1_2022)) |>
  flextable() |>  
  theme_vanilla() |> 
  bg(bg = "grey", part = "header") |> 
  align(align = "center", part = "all")|>
...
bn.sum.data <- tidy.bn.data |> 
  group_by(Year_Month) |> 
  summarize(bn_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            bn_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
#call age
tidy.ep.data$Alter <-
  year(tidy.ep.data$Datum) - year(tidy.ep.data$e_Geb.datum) - (
    month(tidy.ep.data$Datum) < month(tidy.ep.data$e_Geb.datum) |
      (
        month(tidy.ep.data$Datum) == month(tidy.ep.data$e_Geb.datum) &
          day(tidy.ep.data$Datum) < day(tidy.ep.data$e_Geb.datum)
      )
  )


tidy.ep.data <- tidy.ep.data |> relocate(Alter, .before = f_Geschl.)
tidy.ep.data$Year_Month <- format(ymd(tidy.ep.data$Datum), "%Y-%m")
tidy.ep.data <- tidy.ep.data %>% 
  mutate(Quarter = quarter(Datum), Year = year(Datum))
...
ep.sum.data <- tidy.ep.data |> 
  group_by(Year_Month) |> 
  summarize(ep_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            ep_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )



ep.sum.data$TxpUmsatz <- ep.sum.data$ep_TxpUmsatz
...
```
```{r 2021 ep analysis, echo=FALSE, message=FALSE, warning=FALSE}
ep.sum.method <- tidy.ep.data |> 
  mutate(Quarter = paste0("Q", quarter(floor_date(Datum, "quarter"))),
         Year = year(Datum)) |> 
  filter(Year == 2021) |> 
...
hplc.sum.data <- tidy.hplc.data |> 
  group_by(Year_Month) |> 
  summarize(hplc_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            hplc_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
tidy.lcms.data$Alter <-
  year(tidy.lcms.data$Datum) - year(tidy.lcms.data$e_Geb.datum) - (
    month(tidy.lcms.data$Datum) < month(tidy.lcms.data$e_Geb.datum) |
      (
        month(tidy.lcms.data$Datum) == month(tidy.lcms.data$e_Geb.datum) &
          day(tidy.lcms.data$Datum) < day(tidy.lcms.data$e_Geb.datum)
      )
  )

tidy.lcms.data <- tidy.lcms.data |> relocate(Alter, .before = f_Geschl.)
tidy.lcms.data$Year_Month <- format(ymd(tidy.lcms.data$Datum), "%Y-%m")
tidy.lcms.data <- tidy.lcms.data %>% 
  mutate(Quarter = quarter(Datum), Year = year(Datum))
...
lcms.sum.data <- tidy.lcms.data |> 
  group_by(Year_Month) |> 
  summarize(lcms_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            lcms_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
vh.sum.data <- tidy.vh.data |> 
  group_by(Year_Month) |> 
  summarize(vh_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            vh_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
sum.data <- combined.data |> 
  group_by(Year_Month) |> 
  summarize(
    TxpUmsatz_spezChem = sum(Taxpkt., na.rm = TRUE),
    Anz_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Fälle = n_distinct(b_Fallnummer, na.rm = TRUE)
              )
...
sum.data.22 <-  rbind(sum.data.22, c("Total", colSums(sum.data.22[,2:4]))) 
sum.data.22$TxpUmsatz_spezChem <- as.numeric(sum.data.22$TxpUmsatz_spezChem)
...
sum.data.22 |>  
  ...
  #        part = "body") |> 
  set_header_labels(
    TxpUmsatz_spezChem = "TxpUmsatz -Spez. Chemie-",
    Anz_Aufträge = "Aufträge",
    Anz_Fälle = "Patienten (n)"
  ) |> 
  merge_at(j = c(2, 3), part = "header") |> 
  align(align = "center", part = "all")|>
  set_table_properties(width = 0.8, layout = "autofit") |> 
...
```{r quarterly_spezchem, echo=FALSE, message=FALSE, warning=FALSE}
sum.cases.sc.22 <- combined.data |> 
  mutate(Quarter = paste0("Q", quarter(floor_date(Datum, "quarter")), "_", Year = year(Datum))) |>
  filter(Year == 2022) |> 
  summarize(
    TxpUmsatz_spezChem = sum(Taxpkt., na.rm = TRUE),
...
sum.cases.sc.22 <- sum.cases.sc.22 |> 
  pivot_longer(!Quarter, names_to = "Umsätze", values_to = "col2") %>% 
  pivot_wider(names_from = "Quarter", values_from = "col2")
...
sum.data <- combined.data |> 
  group_by(Year_Month) |> 
  summarize(
    TxpUmsatz_KC = sum(TxpUmsatz, na.rm = TRUE),
    Aufträge_KC = sum(sum(AU_Aufträge, na.rm = TRUE), 
                      sum(DxI_Aufträge, na.rm = TRUE),
                      sum(ep_Aufträge, na.rm = TRUE), 
                      sum(bn_Aufträge, na.rm = TRUE),
                      sum(hplc_Aufträge, na.rm = TRUE),
                      sum(lcms_Aufträge, na.rm = TRUE),
                      sum(vh_Aufträge, na.rm = TRUE)),
    Anz_Patienten_sum = sum(Anz_Patienten, na.rm = TRUE))
...
sum.data.22 |>  
  ...
  #        part = "body") |> 
  set_header_labels(
    TxpUmsatz_KC = "TxpUmsatz",
    Aufträge_KC = "Aufträge KC",
    Anz_Patienten_sum = "Patienten (n)"
  ) |> 
  # merge_at(j = c(2, 3), part = "header") |> 
  align(align = "center", part = "all")|>
...
<br>
```{r sum_all_21, echo=FALSE, message=FALSE, warning=FALSE}
combined.cases.all <- bind_rows(
   tidy.AU.data,
   tidy.dxi.data,
   tidy.ep.data, 
   tidy.bn.data,
...
combined.cases.all$Methode <- as.numeric(combined.cases.all$Methode) 
methoden.kc <- read_excel("MethodenKatalogKC.xlsx", col_types = c("text", "numeric", "text"))
combined.data.kc <- left_join(combined.cases.all, methoden.kc, by = c("Bezeichnung", "Methode"), keep = FALSE, multiple = "any")
write_excel_csv(combined.data.kc, "Combined_Data_KC.csv")

sum.cases.all.21 <- combined.cases.all |> 
  mutate(Quarter = paste0("Q", quarter(floor_date(Datum, "quarter")), "_", Year = year(Datum))) |>
  filter(Year == 2021) |> 
  summarize(
    TxpUmsatz_KC = sum(Taxpkt., na.rm = TRUE),
    Auftragstaxen = n_distinct(b_Fallnummer, na.rm = TRUE)*10,
    Umsatz_CHF = sum(Taxpkt., na.rm = TRUE) + n_distinct(b_Fallnummer, na.rm = TRUE)*10,
    Anz_Aufträge = n_distinct(a_Tagesnummer, na.rm = TRUE),
    Anz_Fälle = n_distinct(b_Fallnummer, na.rm = TRUE),
    .by = Quarter
  )

sum.cases.all.21.piv <- sum.cases.all.21 |> 
  pivot_longer(!Quarter, names_to = "Umsätze", values_to = "col2") %>% 
  pivot_wider(names_from = "Quarter", values_from = "col2")
...
```{r sum_all_22, echo=FALSE, message=FALSE, warning=FALSE}
# combined.cases.all <- bind_rows(
#   tidy.AU.data,
#   tidy.dxi.data,
#   tidy.ep.data, 
...
sum.cases.all.22 <- combined.cases.all |> 
  mutate(Quarter = paste0("Q", quarter(floor_date(Datum, "quarter")), "_", Year = year(Datum))) |>
  filter(Year == 2022) |> 
  summarize(
    TxpUmsatz_KC = sum(Taxpkt., na.rm = TRUE),
...
sum.cases.all.22$`Delta 22vs21(%)` <- as.integer(sum.cases.all.22$TxpUmsatz_KC*100/sum.cases.all.21$TxpUmsatz_KC-100)



sum.cases.all.22.piv <- sum.cases.all.22 |> 
  pivot_longer(!Quarter, names_to = "Umsätze", values_to = "col2") %>% 
  pivot_wider(names_from = "Quarter", values_from = "col2")


sum.cases.all.22.piv |>  
  flextable() |>  
  theme_vanilla() |> 
  bg(bg = "grey", part = "header") |> 
  align(align = "center", part = "all")|>
