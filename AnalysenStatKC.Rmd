...
AU.sum.data <- tidy.AU.data |> 
  ...
  summarize(AU_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            AU_Auftraege = n_distinct(a_Tagesnummer, na.rm = TRUE),
...
dxi.sum.data <- tidy.dxi.data |> 
  ...
  summarize(DxI_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            DxI_Auftraege = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
            )
...
sum.data <- combined.data |> 
  ...
  summarize(
    ...
    Auftraege_Hochd. = n_distinct(a_Tagesnummer, na.rm = TRUE), 
...
bn.sum.data <- tidy.bn.data |> 
  ...
  summarize(bn_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            bn_Auftraege = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
global.ep.data <- global.ep.data  |> 
  mutate_at(vars(!a_Tagesnummer:h_Auftragg.), ~as.numeric(str_replace_all(., c("s.unten" = "1", 
                                    "folgt" = "1", 
                                    "s. unten" ="1", 
                                    "negative" ="1", 
                                    "fraglich" = "1",
...
ep.sum.data <- tidy.ep.data |> 
  ...
  summarize(ep_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            ep_Auftraege = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(c_Fallnummer, na.rm = TRUE)
  )
...
hplc.sum.data <- tidy.hplc.data |> 
  ...
  summarize(hplc_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            hplc_Auftraege = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
lcms.sum.data <- tidy.lcms.data |> 
  ...
  summarize(lcms_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            lcms_Auftraege = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(c_Fallnummer, na.rm = TRUE)
  )
...
vh.sum.data <- tidy.vh.data |> 
  ...
  summarize(vh_TxpUmsatz = sum(Taxpkt., na.rm = TRUE),
            vh_Auftraege = n_distinct(a_Tagesnummer, na.rm = TRUE),
            Anz_Patienten = n_distinct(b_Fallnummer, na.rm = TRUE)
  )
...
sum.data.22 |>  
  ...
  set_header_labels(
    ...
    Anz_Auftraege = "Auftraege",
    Anz_Faelle = "Patienten (n)"
  ) |> 
  merge_at(j = c(2, 3), part = "header") |> 
  align(align = "center", part = "all")|>
  set_table_properties(width = 0.8, layout = "autofit") |> 
