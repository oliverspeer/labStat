...
setnames(DT.customer, "VAXKUERZEL", "Auftragg.")
...
fun.read.multi.excel.data <- function(file.pattern, dt.name) {
  ...
  for (file.name in files) {
    ...
    head2 <- readxl::read_excel(full.path, skip = 1, col_names = TRUE, 
...
# # testing--------------------------------------------------------
# test.dt <- AU.data1[!is.na(`GFR(CKD-EPI)_22125`), ]
# test.dt1 <- AU.data1[!is.na(`GFR(CKD-EPI)_22125`), ]
# test.dt2 <- AU.data1[!is.na(`GFR(CKD-EPI)_22125`), ]
# nrow(test.dt)
# nrow(test.dt1)
# nrow(test.dt2)
# nrow(test.dt1) + nrow(test.dt2) == nrow(test.dt)
# nrow(AU.data1)
...
# keep rows that are unique in a_Tagesnummer and b_Fallnummer and Methode and Werte
DT.kc.blood <- DT.kc.blood[!duplicated(DT.kc.blood, by = c("Tagesnummer", "Fallnummer", "Methode", "Werte"))]


# Control for doubled data entries----------------------------------------
# Count the number of measurements for each Tagesnummer and Bezeichnung combination
counts <- DT.kc.blood[, .N, by = .(Tagesnummer, Bezeichnung)]

# Filter to find combinations with more than one measurement
(multiple_measurements <- counts[N > 1])

# Count unique Tagesnummer with multiple measurements
(unique_events_with_multiple_measurements <- unique(multiple_measurements$Tagesnummer))

# Number of unique Tagesnummer with multiple measurements
num_unique_events_multiple_measurements <- length(unique_events_with_multiple_measurements)

num_unique_events_multiple_measurements


# delete rows with double daly numbers
DT.kc.blood <- DT.kc.blood[!Tagesnummer %in% unique_events_with_multiple_measurements]
