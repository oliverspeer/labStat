...
query <- sprintf("SELECT MeasurementData.Jahr AS Year, 
                         MAX(MeasurementData.Datum) as Datum,
                       SUM(Txp) AS 'Txp Umsatz', 
                       COUNT(DISTINCT Tagesnummer) AS 'Anzahl Aufträge',
                       COUNT(DISTINCT Fallnummer) AS 'Anzahl Fälle'
                  FROM MeasurementData
     ...
     setDT(data.device.y)
     data.device.y$Datum <- as.Date(data.device.y$Datum, format = "%Y-%m-%d")
     ...
     if(nrow(data.device.y) >1) {
       ...
       #yearFraction <- yday(today()-11) / yday(as.Date(paste(year(today()), "12-31", sep = "-")))
       yearFraction <- yday(data.device.y$Datum[data.device.y$Year == year(today())]) / yday(as.Date(paste(year(today()), "12-31", sep = "-")))
